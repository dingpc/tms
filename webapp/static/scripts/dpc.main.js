		...
		success : function(data) {
			if(data) {
				if(data.success == "true") {
					...
					alert("保存失败！\n数据重复，请重新填写！");
				} else if(data.success == "passwordError") {
					alert("原密码错误！");
				} else {
					alert("保存失败！");
				}
 ...
 * Ajax方式删除选择数据
 */
function ajax_deleteSelect(checkboxName, attr_name, url) {
	var idStr = getCheckBoxData(checkboxName, attr_name);
	if(idStr == "") {
		alert("请选择需要删除的数据！");
		return false;
 ...
 * @param url  跳转地址.
 */	
function location_href_select(checkboxName, attr_name, url){
	var idStr = getCheckBoxData(checkboxName, attr_name);
	if(idStr == "") {
		alert("请选择数据！");
		return false;
...
function validateForm(formId) {
	...
	$("#" + formId).find(".form-control").each(function() {
		if ($(this).attr("data-verify") == undefined) {
			return true;
		}
		var boe_options = eval("({" + $(this).attr("data-verify") + "})");

		var class_ = $(this).attr('class');
		var obj = this;
		var value = $.trim($(this).val());
		var text = $(this).parent().parent().find("label").eq(0).attr("for");
...
/*function validateForm(formId) {
	var validation = true;
	//$("#" + formId).find("input[class^='inputText'],textarea[class^='inputText'],.selectlist,.inputTextarea,.Wdate,.htmleditor,select[class^='easyui-combobox']")
	//$("#" + formId).find("input[class^='form-control'],textarea[class^='form-control'],.form-control,.selectlist,.inputTextarea,.Wdate,.htmleditor,select[class^='easyui-combobox']")
	$("#" + formId).find(".form-control")
	.each(function() {
		if ($(this).attr("data-verify") == undefined) {
			return true;
		}
...
function openSelect(checkboxName, attr_name, url) {
	var idStr = getCheckBoxData(checkboxName, attr_name);
	if(idStr == "") {
		alert("请选择数据！");
		return false;
	...
	url = url + idStr;
	openNewWin({url:url,width:1000,height:600,winName:'select'});
}

function ajax_select_submit(checkboxName, attr_name, id_name, url) {
	var idStr = getCheckBoxData(checkboxName, attr_name);
	$.ajax({
		type : 'POST',
		url : url,
		data : 'idStr=' + idStr + '&' + id_name + '=' + $('#' + id_name).val() + '&date='+ Math.random(),
		dataType : 'json',
		success : function(data) {
			if(data) {
				if (data.success == "true") {  
...
function back_home() {
	...
	if(window.top != window.self) {
		window.top.location.href = "/login/home";
	}
}

function location_search(url, searchForm, pageOffset) {
	var dataStr = $("#" + searchForm).serialize();
	dataStr = decodeURIComponent(dataStr,true);
	dataStr = dataStr + "&pageOffset=" + pageOffset;
	//dataStr = "searchQuestionTitle=" + encodeURI(encodeURI($("#searchQuestionTitle").val())) + "&currentPage=" + $("#currentPage").val() + "&pageOffset=" + pageOffset;
	//alert(dataStr);
	//location_href(url + "?" + dataStr);
	location_href(encodeURI(url + "?" + dataStr));
}
