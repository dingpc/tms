...
public class StringUtil {
	...
	public static String listToString(List<String> stringlist, String splitWord) {
		String resultString = "";
	...
	/**
	 * 格式化字符串，如果为空，则返回指定的默认值；如果非空，encode为true，则返回UTF-8编码的字符串，否则返回原字符串
	 * @param sourceStr
	 * @param defaultStr
	 * @param encode
	 * @return
	 */
	public static String formateStringToUTF8(String sourceStr, String defaultStr, boolean encode) {
		if (isEmpty(sourceStr)) {
			return defaultStr;
		} else {
			if(encode) {
				try {
					return new String(sourceStr.getBytes("ISO8859-1"), "UTF-8");
	...
	public static int formateStringToInt(String sourceStr, int defaultInt) {
		if (isEmpty(sourceStr)) {
			return defaultInt;
		} else {
